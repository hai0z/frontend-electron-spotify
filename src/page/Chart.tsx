...
const ChartPage = () => {
  ...
  const { data: chartHome, isFetching } = useQuery({
    queryKey: ["chartHome"],
  ...
  const handleButtonPlay = () => {
    if (playlist.encodeId == "Chart") {
      setIsPlaying(!isPlaying);
    } else {
      handlePlaySong(chartHome?.RTChart?.items[0], {
        encodeId: "Chart",
        songs: chartHome?.RTChart?.items,
        title: "Bảng xếp hạng",
      });
    }
  };
              ...
              showHeaderContent ? (
                <AnimatePresence>
                  ...
                  >
                    ...
                    >
                      {isPlaying && playlist.encodeId == "Chart" ? (
                        <BsFillPauseFill
                          className="text-[28px]"
                          color="oklch(var(--pc))"
                        />
          ...
          <div className="-mt-64 mx-6 flex flex-col" ref={stickyRef}>
            <div>
              <div className=" span-lg font-semi-bold flex  mt-8 justify-between items-center flex-row">
                <div className=" font-bold text-2xl mb-2">Bảng xếp hạng</div>
              </div>
              <div>
                {chartHome?.RTChart?.items?.map((pl: any, index: number) => (
                  <motion.div
                    onContextMenu={(e) => {
                      setSelectedTrack(pl);
                      displayMenu(e);
